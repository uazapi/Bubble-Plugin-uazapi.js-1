function(instance, properties, context) {
  ...
  var url = baseUrl + "/automate/fetchConversation/" + instancia;
  ...
  if (properties.name && properties.name.trim() !== "") {
    raw["name"] = properties.name.trim();
  }
  
  if (properties.status && properties.status.trim() !== "") {
    raw["status"] = properties.status.trim();
  }
  
  if (properties.remoteJid && properties.remoteJid.trim() !== "") {
    raw["remoteJid"] = properties.remoteJid.trim();
  }
...
fetch(url, requestOptions)
  ...
  .then(resultObj => {
      ...
      if (Object.keys(resultObj).length > 0) {
          ...
          instance.publishState('logs', resultObj);
